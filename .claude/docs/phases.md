...
- Prisma schema with `users`, `units`, `audit_logs` tables

---

## Phase 2 — Maintenance Fees & PhonePe Payments
...
- `fee_schedules`, `payments` Prisma tables

---

## Phase 3 — Maintenance Issue Tracker
...
- `/app/(dashboard)/president/issues/` — manage all issues
- `/app/api/issues/` — CRUD + status transition endpoints
- `issues`, `issue_comments` Prisma tables
- Supabase Storage bucket for issue photos

---

## Phase 4 — Expenses, Balance Sheet & Reports
...
- `announcements`, `documents` Prisma tables

---

## Phase Order Rationale

Phases are ordered by value delivered:
1. Nothing works without auth and users (Phase 1)
2. Money collection is the primary pain point (Phase 2)
3. Issue tracking is the second most requested (Phase 3)
4. Reports validate everything built so far (Phase 4)
